defmodule MU.Link do
  ...
  import MU.Regex
    ...
    defp host do
      Application.get_env(:mu_server, :host)
    end
    ...
    def siteLink(text) do
      Regex.replace(site_link_regex(), text,  " <a href=\"#{host()}/site/\\1\" target=\"_blank\">\\2</a> ")
    end
  ...
  # https://lookupnote.herokuapp.com/notes/439?index=0&previous=439&next=439&id_list=439
   ...
   def formatXREF(text) do
     Regex.replace(xref_regex(), text, "<a href=\"#{LookupPhoenix.Global.host()}/xnotes/\\1\">\\2</a>")
   end
