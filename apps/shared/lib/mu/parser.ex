...
#####################################
  ...
  def type_of_line3(line, symbol) do
    cond do
      ...
      symbol != :verbatim && Enum.member?(["--", "----"], line) -> :block_separator
      symbol == :verbatim && line == "----" -> :verbatim_separator
      true -> :ordinary_line
    end
  end
...
"""
  ...
  def ns3b(line, state) do
    ...
    # start
     ...
     {:start, :verbatim_start} == {symbol, line_type} ->
       %{ state | symbol: :verbatim_body}
